			...
			if (uid !== null) {
				...
				if (docSnapshot.exists() && docSnapshot.data().uid === uid) {
					...
					if (Array.isArray(arrayField)) {
						const elementsToDeleteArray = Array.isArray(elementsToDelete)
							? elementsToDelete
							: [elementsToDelete];
									...
									if (attribute === 'createdAt') {
										...
										return (
											createdAtTimestamp.seconds ===
												(elementsToDeleteArray[index] as any).seconds &&
											createdAtTimestamp.nanoseconds ===
												(elementsToDeleteArray[index] as any).nanoseconds
										);
									}
						...
						console.log(
							`필드 '${fieldName}'의 일부 원소가 성공적으로 삭제되었습니다`
						);
					} else {
						const updateData = {};
