	...
	const UpdateField = async (data, documentId, authorized = true) => {
		try {
			if (uid !== null) {
				...
				if (
					docSnapshot.exists() && authorized
						? docSnapshot.data().userId === uid
						: true
				) {
					const updateData = {};
			...
			if (uid !== null) {
				...
				if (docSnapshot.exists() && docSnapshot.data().userId === uid) {
					...
					if (Array.isArray(arrayField)) {
						const elementsToUpdateArray = Array.isArray(elementsToUpdate)
							? elementsToUpdate
							: [elementsToUpdate];
							...
							(item) =>
								...
								attributesToUpdate.every((attribute) => {
									if (attribute === 'createdAt') {
										...
										return (
											createdAtTimestamp.seconds ===
												elementsToUpdateArray[1].seconds &&
											createdAtTimestamp.nanoseconds ===
												elementsToUpdateArray[1].nanoseconds
										);
									}
						...
						if (indexToUpdate !== -1) {
							const updatedItem = {
								...arrayField[indexToUpdate],
								[targetAttribute]: newValue,
							};
							...
							await updateDoc(docRef, updateData);
							console.log(
								`'${fieldName}' 필드의 특정 객체의 '${targetAttribute}' 속성이 성공적으로 수정되었습니다.`
							);
						} else {
